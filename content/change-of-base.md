...
Let $n > 0$ and let $\log_a n = p$, $\log_b n = q$ be its logarithms in two different systems. By definition $a^p = n = b^q$, hence $a^p = b^q$, so $a = b^{q/p}$. The ratio $p/q$ is therefore the same constant for *every* $n$ — call it $\lambda = p/q = \log_b a$ (source: chapter6.pdf, §107).
...
Multiplying every common (base-10) logarithm by $3.3219277$ produces the corresponding base-2 logarithm (source: chapter6.pdf, §107). The "golden rule for logarithms," in Euler's phrase.
...
- Equations of the form $a^x = b$ have the base-independent solution $x = \log b / \log a$ (source: chapter6.pdf, §111) — the ratio is the same in any system, so one can use the most convenient table.
...
A direct corollary of the product rule and §107: only the logarithms of *primes* need be computed by the slow geometric-mean method. Logarithms of composites follow by addition (source: chapter6.pdf, §109):
