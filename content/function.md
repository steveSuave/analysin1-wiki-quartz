...
> A function of a variable quantity is an analytic expression composed in any way whatsoever of the variable quantity and numbers or constant quantities. (source: chapter1, §4)
...
Since any value may be substituted for the variable, the function takes on infinitely many values. No value is excluded, because the variable admits complex values as well as real (source: chapter1, §5). The function $\sqrt{9 - z^2}$ is bounded by $3$ on real inputs but attains any prescribed value on complex inputs; for example at $z = 5i$ it equals $\sqrt{34}$ in the real direction Euler uses in the text.
...
The fundamental operations are (source: chapter1, §6):
...
Euler includes the "solution of equations" as a primary operation to account for quantities that are determined by a variable but cannot be expressed through the first six operations alone. For example, if $Z$ is defined by $Z^5 = az^2 Z^3 - bz^4 Z^2 + cz^3 Z - 1$, then $Z$ is a function of $z$ because its value is determined by the value of $z$, even if "common algebra" lacks the means to solve for $Z$ explicitly using radicals (source: chapter1, §8). This allows Euler to classify such quantities as algebraic functions rather than transcendental.

## Reciprocity

If $y$ is a function of $z$, then by rearranging the defining equation, $z$ is a function of $y$ (source: chapter1, §16). The number of values each takes in terms of the other can differ: if $y^3 = ayz - bz^2$, then $y$ is a three-valued function of $z$ while $z$ is a two-valued function of $y$.

If $y$ and $x$ are both functions of $z$, eliminating $z$ from the two defining equations expresses each as a function of the other (source: chapter1, §17).
