...
(source: chapter11.pdf, §191; the algebra of the lead term is finicky but not deep). Similarly for cosine:
...
Both sets are [[zeta-at-even-integers|§169 sums]], differing by the factors $(1 - 2^{-2j})\zeta(2j)$ vs $2^{-2j}\zeta(2j)$. Euler tabulates both to 16+ digits (source: chapter11.pdf, §193). Note that $A_j + 2^{-2j}\zeta(2j) = \zeta(2j)$ provides an internal consistency check.
...
(source: chapter11.pdf, §192–§194). The "$- 1/4^j$" subtraction inside each $\alpha, \beta, \ldots$ accounts for the $k = 1$ term of the all-integer even-power sum being absent (it is folded into the closed-form lead $\log m + \log(2n - m) + \log(2n + m) - 3\log n + \log\pi - \log 8$).
...
(source: chapter11.pdf, §192–§194). Same recipe: lead term absorbs the $k = 0$ slot, and each $(A_j - 1) = A_j - 1/1^{2j}$ is the *odd*-integer reciprocal-power sum minus its first entry.
...
Transposing the resulting double sum, the columns are again $A, B, C, \ldots$. Result (source: chapter11.pdf, §198):
...
(source: chapter11.pdf, §198). $\sec$ and $\csc$ then follow by simple addition/subtraction of $\tan$ and $\cot$ using the §137 identities.

## Why this matters

These five formulas — $\log\pi$, $\log\sin$, $\log\cos$, $\tan$, $\cot$ — share *one* table of constants $A, B, C, \ldots, \alpha, \beta, \gamma, \ldots$. Euler computes that table once (chapter11.pdf, §190 and §193) and reuses it for every trig log computation thereafter. The combined effect is to make a thirty-digit *Tabula Sinuum, Tangentium et Secantium logarithmica* feasible to compute by hand.
