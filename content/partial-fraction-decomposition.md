...
$$ \frac{M}{N} = \sum_{i} \frac{A_i}{p_i - q_i z} \qquad (\text{source: chapter2.pdf, \S 40}). $$
...
Euler's derivation: from $M/N = A/(p - qz) + P/S$ one gets $M - A S = (p - q z) P$, which vanishes at $z = p/q$, forcing $A = M/S$ evaluated there (source: chapter2.pdf, §41).
...
5. Continue until all $n$ numerators are obtained (source: chapter2.pdf, §45).
...
5. Sum all simple fractions (plus the polynomial part, if any). The result equals $M/N$ in its simplest form (source: chapter2.pdf, §46).
...
There is no polynomial part because the given function is proper (source: chapter2.pdf, §46).
