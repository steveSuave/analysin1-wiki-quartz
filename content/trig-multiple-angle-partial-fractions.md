...
i.e., $n\cot nz = \cot z + \cot\!\bigl(\tfrac{2\pi}{n}+z\bigr) + \cdots$ — a sum of $n$ cotangents (source: chapter14, §250).
...
(source: chapter14, §249). For odd $n$, the numerator has degree $n$ and the denominator has degree $n-1$. The $n$ roots of the numerator (when $\tan nz$ is set to a fixed value) are $\tan z, \tan(\frac{\pi}{n}+z), \tan(\frac{2\pi}{n}+z), \ldots$
